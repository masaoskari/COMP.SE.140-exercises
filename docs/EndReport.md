...
| 1.0     | ??date?? | Add more descriptions and some minor improvements for final submission         |
...
Application can be fully tested locally. This serction contains the information to test apllication locally. There are also instructions how to do linting and run unit and e2e tests locally.
...
#### Linting
Linting jobs for both TypeScript and Python runs automatically in GitLab pipeline. Those can be tested locally also with following steps (NOTE! depending on your package manager system and node or python version the steps might vary).
...
#### Unit tests
For service1 there are unit tests made with Jest framework. These can be run locally with following steps (NOTE! commands may vary if you use different package manager):
...
```
Basically with `TEST_COMMAND` environmental variable you can configure which tests is run. That variable is also used in pipeline to run different tests.
...
To get DinD working in registered GitLab runner the runner needs to use privileged mode. That can be enabled with setting `priviledged = true` in runner configuration file. See more [details](https://docs.gitlab.com/ee/ci/docker/using_docker_build.html#use-docker-in-docker). 
